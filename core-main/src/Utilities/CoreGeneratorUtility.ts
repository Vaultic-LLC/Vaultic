...
import { ICoreGeneratorUtility } from '@vaultic/shared/Types/Utilities';
...
export default class CoreGeneratorUtility implements ICoreGeneratorUtility
{
    ...
    generateRandomPasswordOrPassphrase(type: RandomValueType, length: number, includeNumbers: boolean, includeSpecialCharacters: boolean,
        includeAbmiguousCharacters: boolean, passphraseSeperator: string): string
    {
        let randomValue = "";
        if (type == RandomValueType.Password)
        {
            let possibleCharacters = "abcdefghjkmnpqrstuvwxyzABCDEFGHJKMNPQRSTUVWXYZ";
            if (includeNumbers)
            {
                possibleCharacters += "23456789";
            }
            ...
            if (includeAbmiguousCharacters)
            {
                possibleCharacters += "10oOiIlL|!"
            }
            ...
            if (includeNumbers && includeSpecialCharacters && length >= 16)
            {
                ...
                while (isWeak)
                {
                    ...
                    let randomValues = environment.utilities.crypt.randomBytes(length);
        ...
        else
        {
            for (let i = 0; i < length; i++)
            {
                ...
                if (i == 0)
                {
                    randomValue += passphraseWordList[digits];
                }
                else
                {
                    randomValue += `${passphraseSeperator}${passphraseWordList[digits]}`;
                }
            }
        }
...
const passphraseSpecialCharactersLow = 66632;
const passphraseNumbersLow = 65434;

export const passphraseWordList: { [key: string]: string } = {
	"11111": "abacus",
	"11112": "abdomen",
	"11113": "abdominal",
	"11114": "abide",
