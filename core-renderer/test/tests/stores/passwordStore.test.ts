...
{
    return app.currentVault.passwordStore.passwords.filter(p => !p.isWeak && !app.currentVault.passwordStore.duplicatePasswords.value.includes(p.id) && !p.containsLogin && !p.isOld);
}
...
passwordStoreSuite.tests.push({
    name: "PasswordStore Metrics Work After Add", func: async (ctx: TestContext) =>
    {
        ...
        const retrievedDuplicatePasswordOne = app.currentVault.passwordStore.duplicatePasswords.value.filter(p => p == duplicatePassword.id);
        const retrievedDuplicatePasswordTwo = app.currentVault.passwordStore.duplicatePasswords.value.filter(p => p == weakPassword.id);
...
passwordStoreSuite.tests.push({
    name: 'PasswordStore Update works', func: async (ctx: TestContext) => 
    {
        ...
        const oldPassword = app.currentVault.passwordStore.passwords.filter(p => p.login == originalLogin);
...
passwordStoreSuite.tests.push({
    name: 'Update security question works', func: async (ctx: TestContext) =>
    {
        ...
        await app.currentVault.passwordStore.updatePassword(masterKey, password, false, [securityQuestion.id], [securityQuestion.id]);
        const updatedPassword = app.currentVault.passwordStore.passwords.filter(p => p.id == password.id);
...
passwordStoreSuite.tests.push({
    name: "PasswordStore Metrics Work After Update", func: async (ctx: TestContext) =>
    {
        ...
        let retrievedDuplicatePasswordOne = app.currentVault.passwordStore.duplicatePasswords.value.filter(p => p == duplicatePassword.id);
        let retrievedDuplicatePasswordTwo = app.currentVault.passwordStore.duplicatePasswords.value.filter(p => p == weakPassword.id);
        ctx.assertEquals("Duplicate password one exists", retrievedDuplicatePasswordOne.length, 1);
        ctx.assertEquals("Duplicate password two exists", retrievedDuplicatePasswordTwo.length, 1);
        ...
        retrievedDuplicatePasswordOne = app.currentVault.passwordStore.duplicatePasswords.value.filter(p => p == duplicatePassword.id);
        retrievedDuplicatePasswordTwo = app.currentVault.passwordStore.duplicatePasswords.value.filter(p => p == weakPassword.id);
...
passwordStoreSuite.tests.push({
    name: "PasswordStore Update With Groups Works", func: async (ctx: TestContext) =>
    {
        ...
        let retrievedPassword = app.currentVault.passwordStore.passwords.filter(p => p.id == addedGroupPassword.id)[0];
        ...
        retrievedPassword = app.currentVault.passwordStore.passwords.filter(p => p.id == removedGroupPassword.id)[0];
